  ...
  methods: {
    ...
    onTreeRowClick(treeData) {
      if (
        ...
        this.$refs.dictValCon.emptyInputs();
    ...
    dictDefOperate(data) {
      let otherLang = i18nService.getOtherLanguageMap();
      let isAddition = _.isEmpty(data);   
      let dialog = this.$refs.dialog;
      let operate = "create";
      
      let form = { dictCode: "", dictName: "" };
      let list = [ { name: "Name", prop: "dictName" }, { name: "Code", prop: "dictCode" } ];
      let rules = { 
      ...
      return dialog
        .show({
          ...
          rules,
          list,
          title: isAddition? "Add DictDefinition" : "Edit DictDefinition"
        })
        .then(result => {
          this.showLoading();
    ...
    parseDictValDialogConfig(isAddition, form) {
      //this.dictionaryValTableData.rows
      let parseParentList = () => {
        return _.map(this.dictionaryDefTableData.rows, row => {
          return { name: row.dictName, value: row.dictCode }
        })
      };
      let otherLang = i18nService.getOtherLanguageMap();
      ...
      list = _.map(list, one => {
        ...
        if (one.prop === "parentValueCode") {
          ...
          if(isAddition) {
            form[one.prop] = this.selectedDictDefCode
          }
        }
