<template>
  <div class="panel role-assign-wrapper">
    ...
    <div :class="{ 'roles-wrapper':true, 'roles-empty':_.isEmpty(roles) }" v-loading="loading">
      <el-tree
        :data="roles"
...
export default {
  ...
  methods: {
    ...
    showUserSelection(selectedUsers, roleId) {
      ...
      this.loading = true;
      this.confirmTxt = lang["platform.common.ok"];
      this.cancelTxt = lang["platform.common.cancel"];
      ...
      this.fetchUsers()
        ...
        .catch((error) => {
          this.loading = false;
          this.$error(error.message || otherLang["operateError"]);
        });
    },
    ...
    renderTreeContent(h, { node, data, store }) {
      ...
      return (
        <div class={rowCls}>
          ...
          {isRoleRow && <span class="role-desc-label" title={data.desc}>{data.desc}</span>}
          {isRoleRow && (
    ...
    remove(data, e) {
      ...
      e.stopPropagation()
      this.$alert(
        otherLang["deleteExtraWarning"].replace('{{}}',lang['platform.common.members']),
        "Warning"
      ).then(() => {
    ...
    goEdit(row) {
      ...
      this.$router.push({ path: `/app/role/edit/${row.id}` });
    },

    goProfileEdit(row) {
      baseStore.set("role-row-data", row);
      this.$router.push({ path: `/app/profile/edit/${row.id}`})
    },
    ...
    addRole() {
      this.$router.push({ path: "/app/role/edit/0" });
    }
  }
};
</script>
