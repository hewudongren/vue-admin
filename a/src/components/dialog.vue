<template>
<el-dialog :title="title" :visible.sync="visiable" :close="onClose" :top="top">
  <el-form ref="form" :model="form" :rules="rules">
    <el-form-item v-for="(item,i) in list" v-if="item.type==undefined||item.type=='input'"
      ...
      <el-input v-else v-model="form[item.prop]" auto-complete="off" :placeholder="item.placeholder || item.name" :maxlength="maxlength" />
    </el-form-item>
    <el-form-item v-else-if="item.type=='select'" :key="i" :label="item.name" :prop="item.prop" label-width="100px">
      <el-select v-model="form[item.prop]" :disabled="readOnly" :placeholder="item.placeholder || item.name">
